...
void initCmds(CmdMgr *mgr) {
  ...
  Cmd *read_graphCmd = new Read_GraphCmd("read_graph");
  mgr->regCmd(read_graphCmd);

  Cmd *write_tree_dfsCmd = new Write_Tree_DFSCmd("write_tree_dfs");
  mgr->regCmd(write_tree_dfsCmd);

  Cmd *write_tree_bfsCmd = new Write_Tree_BFSCmd("write_tree_bfs");
  mgr->regCmd(write_tree_bfsCmd);

  Cmd *write_dijkstraCmd = new Write_DijkstraCmd("write_dijkstra");
